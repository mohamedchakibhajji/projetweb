...
class CommandefC 
{
	...
	function incDatec($referenceC){
		$sql="update commandef set datec = datec+2 where referenceC= '$referenceC'";
		$db = config::getConnexion();
        $req=$db->prepare($sql);
		$req->bindValue(':referenceC',$referenceC);
		try{
...
try{		
  ...
  print_r($datas);
	...
	function rechercherCommandef2($recherche){
$sql="SELECT * from commandef where  referenceC like '%$recherche%' or titre like '%$recherche%' or ref_fournisseur like '%$recherche%'or quantite like '%$recherche%' or datec like '%$recherche%' or etat like '%$recherche%'";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
		return $liste;
